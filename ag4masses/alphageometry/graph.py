...
from geometry import Circle, Line, Point, Segment, SemiCircle
...
class Graph:
  ...
  def add_piece(
    ...
    if name in ['coll', 'collx']:
      ...
      return self.add_semicircle(args, deps)
    elif name == 'cyclic':
  ...
  def check(self, name: str, args: list[Point]) -> bool:
    ...
    if name == 'semicircle':
      return self.check_semicircle(args)
    if name == 'aconst':
  ...
  def get_semicircle_thru_triplet(self, p1: Point, p2: Point, p3: Point) -> SemiCircle:
    p1, p2, p3 = sorted([p1, p2, p3], key=lambda x: x.name)
    if (p1, p2, p3) in self._triplet2semicircle:
        return self._triplet2semicircle[(p1, p2, p3)]
    return self.get_new_semicircle_thru_triplet(p1, p2, p3)

  def get_new_semicircle_thru_triplet(self, p1: Point, p2: Point, p3: Point) -> SemiCircle:
    """Get a new SemiCircle that goes thru three given Points."""
    p1, p2, p3 = sorted([p1, p2, p3], key=lambda x: x.name)
    name = p1.name.lower() + p2.name.lower() + p3.name.lower()
    semicircle = self.new_node(SemiCircle, f'({name})')
    semicircle.num = nm.SemiCircle(p1=p1.num, p2=p2.num, p3=p3.num)
    semicircle.points = p1, p2, p3

    self.connect(p1, semicircle, deps=None)
    self.connect(p2, semicircle, deps=None)
    self.connect(p3, semicircle, deps=None)
    self._triplet2semicircle[(p1, p2, p3)] = semicircle
    return semicircle
  ...
  def add_semicircle(
    ...
    o, a, b, c = points
    add = self.add_cong([o, a, o, b], deps=deps)
    add += self.add_cong([o, b, o, c], deps=deps)
    return add
  ...
  def check_semicircle(self, points: list[Point]) -> bool:

    o, a, b = points

    # Check if points a and b lie on the semicircle boundary
    return self.check_cong([o, a, o, b])
  
  def cyclic_dep(self, points: list[Point], p: Point) -> list[Dependency]:
    for p1, p2, p3 in utils.comb3(points):
        if isinstance(self, SemiCircle):
            # Additional check to ensure the points are within the semicircle
            if self.is_within_boundary(p1) and self.is_within_boundary(p2) and self.is_within_boundary(p3) and self.is_within_boundary(p):
  ...
  def additionally_draw(self, name: str, args: list[Point]) -> None:
    ...
    if name in ['on_semicircle', 'tangent']:
      center, point = args[-2:]
      circle = self.new_node(SemiCircle, f'({center.name},{point.name})')
      circle.num = nm.SemiCircle(center.num, p1=point.num)
      circle.points = center, point
    ...
    if name in ['semicircle']:
      o, c, b, a = args[:4]
      a, b, c = sorted([a, b, c], key=lambda x: x.name.lower())
      semicircle = self.new_node(SemiCircle, f'({o.name},{a.name}{b.name}{c.name})')
      semicircle.num = nm.SemiCircle(center=o.num, p1=a.num, p2=b.num, p3=c.num)
      semicircle.points = (a, b, c)
    
    if name in ['tangent_semicircle']:
      center, point, tangent_point = args[:3]
      semicircle = self.new_node(SemiCircle, f'({center.name},{point.name})')
      semicircle.num = nm.SemiCircle(center.num, p1=point.num)
      semicircle.points = center, point

      tangent = self.new_node(Line, f'tangent({tangent_point.name})')
      tangent.num = nm.Line(tangent_point.num, p1=point.num)
      tangent.points = tangent_point, point
  ...
  def add_clause(
    ...
    # Step 2: draw.
    def range_fn() -> (
        list[Union[nm.Point, nm.Line, nm.Circle, nm.HalfLine, nm.HoleCircle, nm.SemiCircle]]
    ):
      to_be_intersected = []
      for c in clause.constructions:
        cdef = definitions[c.name]
  ...
  def all_semicircles(self) -> Generator[tuple[Point, ...], None, None]:
    for l in self.type2nodes[Length]:
        p2p = defaultdict(list)
        for s in l.neighbors(Segment):
            a, b = s.points
